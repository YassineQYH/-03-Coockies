...
/*  AJOUT 4.2 */if(!empty($_GET['action']) /* Pour récupérer les parametre par l'url et je récupère l'action si elle n'est pas vide et éxiste*/ && $_GET['action'] === 'deconnecter') /*  AJOUT 4.2 */
/*  AJOUT 4.2 */ unset($_COOKIE['utilisateur']); // Supprimé la clé utilisateur avec la méthode :: unset /*  AJOUT 4.2 */ Permettra qu'à la suite de l'éxecution du scripte que $_COOKIE de l'utilisateur soit bien vide. /*  AJOUT 4.2 */
/*  AJOUT 4.2 */setcookie('utilisateur', '', time() -10); /* Si je veux vraiment supprimé le cookie, il faut faire un setcookie dans le passé. */ /* pour la clé utilisateur je veux que tu mettes une valeur qui est vide et ensuite mettre une date dans le passé time() -10 sec */ /*  AJOUT 4.2 */
...
<?php if ($nom): ?> <!-- Si c'est défini -->
    ...
    <!-- AJOUT 4.2 --><a href="cookie-04.2-profil.php?action=deconnecter">Se déconnecter</a><!-- AJOUT 4.2 -->
